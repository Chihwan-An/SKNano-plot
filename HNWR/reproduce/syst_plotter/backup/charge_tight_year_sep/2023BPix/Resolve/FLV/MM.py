...
def run_plot():
    ...
    IS_BLIND     = "SR" in HIST_NAME.upper() or "SR" in os.path.basename(PLOT_OUT_NAME).upper()
    ...
    int_row = ["Total", h_total_mc.GetBinLowEdge(1), h_total_mc.GetBinLowEdge(n_bins + 1), total_mc, 0 if IS_BLIND else total_data]
    ...
    _ee_sig_prefix = _sig_prefix.replace("MM", "EE")
    ...
    for sig_name, sig_row in sig_int_rows.items():
        ...
        ee_sig_tsv = os.path.join(signal_dir, f"{_ee_sig_prefix}_{sig_name.replace('MM', 'EE')}_integrated.tsv")
        with open(ee_sig_tsv, "w") as sf:
            sf.write(full_header + "\n")
            sf.write("\t".join(f"{v:.4f}" if isinstance(v, float) else str(v) for v in full_row) + "\n")
        print(f">> Signal TSV (EE copy) saved as {ee_sig_tsv}")
    ...
    for sig_name in sorted(all_wr_for_channel - set(signal_hists.keys())):
        ...
        with open(ee_sig_tsv, "w") as sf:
            sf.write(full_header + "\n")
            sf.write("\t".join(f"{v:.4f}" if isinstance(v, float) else str(v) for v in full_row) + "\n")
        print(f">> Signal TSV (zero EE copy) saved as {ee_sig_tsv}")
