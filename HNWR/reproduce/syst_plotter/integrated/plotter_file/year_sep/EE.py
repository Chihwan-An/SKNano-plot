    ...
    for fname in all_files:
        ...
        if fname_base.startswith("WR"):
            ...
            if SCRIPT_CHANNEL and not fname_base.endswith(SCRIPT_CHANNEL):
                continue
            all_wr_for_channel.add(fname_base)  # 히스토그램 로드 전에 등록
            ...
            if h:
                ...
                print(f"  [SIG ] {fname:<40}: histogram not found, will write zero TSV")
...
# Stack 생성 (yield 많은 순으로 정렬 → 많은 것이 맨 아래에 쌓임)
    ...
    if args.show_sb and signal_hists:
        ...
        for i in range(1, n_bins + 1):
            ...
            for sig_name in sig_names:
                sig_val  = signal_hists[sig_name].GetBinContent(i)
                sb_ratio = sig_val / math.sqrt(bkg_val)
        ...
        print("\n" + "="*120)
        print(">> Bin-by-bin S/sqrt(B) Ratio  [행=시그널, 열=빈]")
        print("="*120)
        hdr = f"{'Signal':<25}" + "".join(f" | {bh:>{col_w}}" for bh in bin_hdrs)
        print(hdr)
        print("-" * 120)
        for j, sig_name in enumerate(sig_names):
            short = sig_name.replace("WR", "").replace("EE", "").replace("MM", "")
            line  = f"{short + '_S/sqrtB':<25}" + "".join(f" | {sb_ratios[j]:>{col_w}.4f}" for (_, _, _, sb_ratios) in sb_rows)
            print(line)
        print("="*120)
        ...
        print("="*120)
        print(f"{'Category':<25}" + "".join(f" | {bh:>{col_w}}" for bh in bin_hdrs))
        print("-" * 120)
        print(f"{'BKG':<25}" + "".join(f" | {bkg_val:>{col_w}.2f}" for (_, bkg_val, _, _) in sb_rows))
        for j, sig_name in enumerate(sig_names):
            short = sig_name.replace("WR", "").replace("EE", "").replace("MM", "")
            line  = f"{short + '_Sig':<25}" + "".join(f" | {sig_vals[j]:>{col_w}.2f}" for (_, _, sig_vals, _) in sb_rows)
            print(line)
        print("="*120)

        ssqrtb_tsv = f"{PLOT_OUT_NAME}_ssqrtb.tsv"
        with open(ssqrtb_tsv, "w") as tsvf:
            tsv_hdr = "Category" + "".join(f"\tBin{rd[0]}[{rd[1]:.0f},{rd[2]:.0f}]" for (rd, *_) in sb_rows)
            tsvf.write(tsv_hdr + "\n")
            tsvf.write("BKG" + "".join(f"\t{bkg_val:.4f}" for (_, bkg_val, _, _) in sb_rows) + "\n")
            for j, sig_name in enumerate(sig_names):
                short = sig_name.replace("WR", "").replace("EE", "").replace("MM", "")
                tsvf.write(f"{short}_Signal" + "".join(f"\t{sig_vals[j]:.4f}" for (_, _, sig_vals, _) in sb_rows) + "\n")
                tsvf.write(f"{short}_S/sqrtB" + "".join(f"\t{sb_ratios[j]:.4f}" for (_, _, _, sb_ratios) in sb_rows) + "\n")
        print(f">> S/sqrt(B) TSV saved as {ssqrtb_tsv}\n")
    ...
    _year    = os.path.basename(os.path.dirname(os.path.dirname(_script_dir)))        # e.g. "2023BPix"
    ...
    # 히스토그램을 못 불러온 시그널(값=0)도 TSV 생성
    for sig_name in sorted(all_wr_for_channel - set(signal_hists.keys())):
        sig_col_header = f"\t{sig_name}\t{sig_name}_Stat_pct\t{sig_name}_SystUp_pct\t{sig_name}_SystDn_pct\t{sig_name}_TotalUp_pct\t{sig_name}_TotalDn_pct"
        for syst in SYST_LIST:
            sig_col_header += f"\t{sig_name}_{syst}_pct"
        full_header = bg_header + sig_col_header
        zero_sig_row = [0.0] * (1 + 5 + len(SYST_LIST))
        full_row = bg_int_row + zero_sig_row
        sig_tsv = os.path.join(signal_dir, f"{_sig_prefix}_{sig_name}_integrated.tsv")
        with open(sig_tsv, "w") as sf:
            sf.write(full_header + "\n")
            sf.write("\t".join(f"{v:.4f}" if isinstance(v, float) else str(v) for v in full_row) + "\n")
        print(f">> Signal TSV (zero) saved as {sig_tsv}")
    ...
    latex.DrawLatex(0.18, 0.82, args.label1 if args.label1 else "ee")
