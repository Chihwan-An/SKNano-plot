...
def run_plot():
    ...
    for fname in all_files:
        ...
        if h:
            print(f"  [MC  ] {fname:<40}: {h.Integral():12.2f} events")
    ...
    _KEEPER.extend([stack, h_total_mc])
    ...
    for i in range(1, n_bins + 1):
        ...
        if y > 0:
            ...
            print(f"{i:<4} | {range_str:<18} | {y:10.1f} | {data_val:10.0f} | {p_stat:7.1f}% | {p_syst_up:8.1f}% | {p_syst_dn:8.1f}% | {p_total_up:8.1f}% | {p_total_dn:8.1f}%")
    ...
    int_row = ["Total", h_total_mc.GetBinLowEdge(1), h_total_mc.GetBinLowEdge(n_bins + 1), total_mc, -1]
    ...
    if Y_LOG:
        p1.SetLogy()
    y_max = args.ymax if args.ymax else h_total_mc.GetMaximum() * (100 if Y_LOG else 1.5)
    ...
    latex.DrawLatex(0.95, 0.93, "9.68 fb^{-1} (13.6 TeV)")
